...
<Box>
  ...
  return (
      ...
      <Box>
        ...
        >
          Note about rendering objects and arrays
        </Typography>
      </Box>

      <Typography>NOTE: Do not render objects like this as it wont work:</Typography>
...
export default App`}
      ...
      <Typography>The core of the problem is "Objects are not valid as a React child," which means the application is trying to render objects, which is not possible.</Typography>
      <Typography>This way it works:</Typography>
      <Box>
      <SyntaxHighlighter language="javascript" style={okaidia}>
          {`const App = () => {
  const exercises = [
  ...
  return (
    <div>
      <p>{exercises[0].name} {exercises[0].reps}</p>
      <p>{exercises[1].name} {exercises[1].reps}</p>
    </div>
  )
}
...
export default App`}
      ...
      <Typography>We see that exercises is an array that contains two objects. These objects can be accessed like in a normal array by its index (here 0 and 1).
        When rendering the results we still need to get the information from the object itself (name or reps in this case) to be able to render it on screen successfully.
      </Typography>
    </Container>
  )
}

export default RFPart1Material